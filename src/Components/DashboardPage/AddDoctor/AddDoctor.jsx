...
const AddDoctor = () => {
    ...
    const { data: jobs = [] } = useQuery({
        queryKey: ['jobs', 'specialty'],
        queryFn: async () => {
            const res = await fetch('https://doctor-portal-back-end.vercel.app/job');
    ...
    const handleSignUp = (data) => {
        const drName = data.name;
        const drEmail = data.email;
        const drSpecialty = data.specialty;
        ...
        fetch('https://api.cloudinary.com/v1_1/dnhefldpl/image/upload', {
            ...
            .then(data => {
                if (data.url) {
                    ...
                    fetch('https://doctor-portal-back-end.vercel.app/add-doctor', {
                        method: 'POST',
                        headers: {
                        ...
                        .then(data => {
                            if (data.acknowledged) {
                                toast.success(`${doctor.drName} is Successfully Added`);
            ...
            <div className="login-form-2">
                <div className="form">
                    <form onSubmit={handleSubmit(handleSignUp)}>
                        ...
                        <input type="text" placeholder='Enter Doctor Name' {...register('name')} id="name" />
                        <label htmlFor="email">Email</label>
                        <input type="email" placeholder='Enter Doctor email' {...register('email')} id="email" />
                        <label htmlFor="specialty">Specialty</label>
                        <select name="specialty" id="specialty" {...register('specialty')}>
                            {
                                jobs.map(job => <option key={job._id} value={job.name}>{job.name}</option>)
                            }
                        </select>
                        ...
                        <div className="photo-box" onClick={() => document.querySelector('.photo').click()}>
                            ...
                            {
                                avatar ? <img className='preview' src={preview} alt='Doctor' /> : <>
                        ...
                        <button className='theme-btn-2' type="submit">Add</button>
                    </form>
                </div>
            </div>
        </div>
